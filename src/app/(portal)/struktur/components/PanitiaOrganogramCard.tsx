...
export function PanitiaOrganogramCard({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl p-4 w-full max-w-xs sm:w-64 text-center relative group transition-all",
        styles.container,
        className
      )}
    >
      {onEdit && (
        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition no-print">
          <button
            type="button"
            onClick={() => onEdit(panitia)}
            className={cn("p-1 rounded transition", styles.editBtn)}
            aria-label={`Edit ${panitia.nama}`}
          >
      ...
      {isKetua && panitia.user_id && panitia.user_username && (
