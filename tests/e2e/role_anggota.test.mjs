...
describe("Role E2E: Akun Anggota Full Journey (All Pages, Features & Route Guards)", () => {
  let browser;
  let context;
  let page;
  const consoleErrors = [];
  ...
  before(async () => {
    ...
    // 1. Ensure test_anggota exists with active status
    await execute("DELETE FROM admin_users WHERE username = 'test_anggota'");
    await execute(
      `INSERT INTO admin_users (id, username, password, nama, role, status)
       VALUES ('test-anggota-id', 'test_anggota', ?, 'Anggota Pelaksana', 'anggota', 'aktif')`,
      [hashPassword("anggota123")]
    );

    // Clean up previous test task and insert fixture task for Anggota
    await execute("DELETE FROM tugas WHERE id = 'e2e-anggota-task' OR nama_tugas LIKE 'E2E-TEST-ANGGOTA%'");
    await execute(
      `INSERT INTO tugas (id, seksi_id, nama_tugas, deskripsi, status, deadline, is_umum, created_by)
       VALUES ('e2e-anggota-task', (SELECT id FROM seksi LIMIT 1), 'E2E-TEST-ANGGOTA-TUGAS-1', 'Tugas koordinasi anggota lapangan', 'Belum Mulai', '1970-01-01', 1, 'test-anggota-id')`
    );
  ...
  it("1. Authenticate as Anggota & Verify Minimal Allowed Navigation", async () => {
    await page.goto(`${BASE_URL}/login`, { waitUntil: "networkidle" });
    await page.locator('input[autocomplete="username"]').fill("test_anggota");
    await page.locator('input[autocomplete="current-password"]').fill("anggota123");
    ...
    assert.ok(profileText.includes("Anggota"), `Header must display Anggota role/name, got: ${profileText}`);

    // Verify navigation links for Anggota: only Dashboard and Tugas
    ...
    assert.ok(navText.includes("Dashboard") || navText.includes("Beranda"), "Navigation must include Dashboard for Anggota");
    assert.ok(navText.includes("Tugas"), "Navigation must include Tugas for Anggota");
    assert.ok(!navText.includes("Keuangan"), "Navigation must NOT include Keuangan for Anggota");
    assert.ok(!navText.includes("Rundown"), "Navigation must NOT include Rundown for Anggota");
    assert.ok(!navText.includes("Tamu"), "Navigation must NOT include Tamu for Anggota");
    assert.ok(!navText.includes("Struktur"), "Navigation must NOT include Struktur for Anggota");
    assert.ok(!navText.includes("Pengguna") && !navText.includes("Akun"), "Navigation must NOT include Pengguna for Anggota");
  });

  it("2. Tugas per Seksi: Anggota Views Tasks & Updates Status with DB Persistence", async () => {
    await page.goto(`${BASE_URL}/tugas`, { waitUntil: "networkidle" });
    await page.waitForURL("**/tugas", { timeout: 8000 });

    const taskTitle = page.locator('h3:has-text("E2E-TEST-ANGGOTA-TUGAS-1")').first();
    ...
    const dbTask = await queryOne("SELECT status FROM tugas WHERE id = 'e2e-anggota-task'");
    assert.equal(dbTask.status, "Proses", "Task status in DB must update to 'Proses'");

    // Verify reload persistence
    await page.reload({ waitUntil: "networkidle" });
    const reloadedCard = page.locator('h3:has-text("E2E-TEST-ANGGOTA-TUGAS-1")').locator("xpath=ancestor::div[contains(@class, 'rounded-2xl') or contains(@class, 'bg-white')]").first();
    const selectAfter = reloadedCard.locator('select[aria-label*="Ubah status"], select').first();
    assert.equal(await selectAfter.inputValue(), "Proses", "Task status changed by anggota must persist as 'Proses'");
  });

  it("3. Strict Route Guards: Anggota Cannot Access Restricted Pages", async () => {
    ...
    assert.ok(
      ...
      "Anggota must be restricted from /keuangan"
    );
    ...
    assert.ok(
      ...
      "Anggota must be restricted from /rundown"
    );
    ...
    assert.ok(
      ...
      "Anggota must be restricted from /tamu"
    );
    ...
    assert.ok(
      ...
      "Anggota must be restricted from /struktur"
    );
    ...
    assert.ok(
      ...
      "Anggota must be restricted from /pengguna"
    );
  });
  ...
  it("5. Zero Fatal Console Errors for Anggota Role", async () => {
    const fatalErrors = consoleErrors.filter((e) => !e.includes("favicon") && !e.includes("404"));
    assert.deepEqual(fatalErrors, [], `Expected zero console errors for Anggota, got: ${fatalErrors.join("; ")}`);
    console.log("✅ Role Anggota Headed Playwright E2E Test PASSED flawlessly!");
  });
});
